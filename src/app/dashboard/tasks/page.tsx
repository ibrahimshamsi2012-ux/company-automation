...
export default function TasksPage() {
  ...
  const executeTask = async () => {
    ...
    try {
      ...
      // The API returns a JSON object with steps, completed, and timeSaved
      setResult({
        steps: data.steps || ["Analyzing requirement", "Allocating neural resources", "Executing sequence"],
        completed: data.completed || "Task sequence finalized successfully.",
        timeSaved: data.timeSaved || "Estimated 2.5 hours",
      });
    } catch (error) {
      console.error("Task Engine Error:", error);
      setResult({
        error: "Neural Engine link disrupted. Please check your configuration."
      });
    } finally {
          ...
          <AnimatePresence>
            ...
            {result && result.error && (
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="glass-card rounded-[40px] p-10 border-red-500/20 bg-red-500/[0.02] flex items-center space-x-4 text-red-400"
              >
                <AlertCircle size={24} />
                <span className="font-bold">{result.error}</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
