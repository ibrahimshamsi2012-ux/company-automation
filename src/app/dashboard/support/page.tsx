...
export default function SupportPage() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Neural interface error");

      const data = await response.json();
      const responseText = data.text || "I apologize, but I am currently processing a high volume of requests. Please try again in a moment.";
      ...
      const errorMessage: Message = { 
        id: (Date.now() + 1).toString(), 
        role: "bot", 
        content: "Neural link disrupted. Please ensure your AI keys are correctly configured." 
      };
